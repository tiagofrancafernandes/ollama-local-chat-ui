...
export const setBaseUrl = (baseUrl: URL |string | null | undefined = null) => {
    ...
    baseUrl = ifUrl(baseUrl)?.toString()
        || serverBaseUrl
        || ifUrl(localStorage?.getItem('server_base_url'))?.toString()
        || defaultBaseUrl;

    serverBaseUrl = ifUrl(localStorage?.getItem('server_base_url'))?.toString()
        || ifUrl(serverBaseUrl as string|null)?.toString() || defaultBaseUrl;
...
export const setModel = (model: string | null | undefined = null) => {
    ...
    model = String(model)?.trim()
        || currentModel
        || String(localStorage?.getItem('model'))?.trim()
        || defaultModel;
...
export const ollamaService = {
    async ask(prompt: string, model = 'mistral', baseUrl: URL |string | null | undefined = null): Promise<OllamaResponse> {
        try {
